...
public struct PGM118669: Solvable {
    ...
    func solution(_ n: Int, _ paths: [[Int]], _ gates: [Int], _ summits: [Int]) -> [Int] {
        ...
        while !heap.isEmpty {
            ...
            // 2차원 배열 재활용 => 원래 사용하던 걸 1차로 임시 저장, 2차로 최종 결과 저장용으로
            // 2차원 배열로 최종 결과까지 필터링하려면 로직 복잡해짐 => 추가로 1차원 배열 두기 => 추가 배열도 2차원이어야 할 것 같은데 => 마지막엔 최소만 있으면 되서 추가 배열 없이 변수 하나로 최소 추적도 될듯
        }
