...
public struct PGM118669: Solvable {
    ...
    func solution(_ n: Int, _ paths: [[Int]], _ gates: [Int], _ summits: [Int]) -> [Int] {
        ...
        while !heap.isEmpty {
            ...
            // 뽑은 경로를 다시 넣었을 때 문제 발생할 수 있음. => 이차원 배열로 테이블 만들어서 저장 => heap에서 꺼내서 배열에 넣고 이 값을 재사용 => heap에 들어가는 값은 현재 heap값 + 테이블값 => 현재 heap값이 애초에 테이블 값 더해서 반영됨 => heap에 새 값 더하면 오히려 문제 발생 => 더한값 자체만 heap에 넣고 테이블에서 뺀 값을 새로 더해서 heap에 넣지 않음 (테이블 값을 더해서 heap에 넣는다는 뜻임) => 최종 값만 heap에 넣음 (최종 더한 값만, 이전 상태 없어짐)
        }
