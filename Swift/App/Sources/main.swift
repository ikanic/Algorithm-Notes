...
public struct PGM118669: Solvable {
    ...
    func solution(_ n: Int, _ paths: [[Int]], _ gates: [Int], _ summits: [Int]) -> [Int] {
        ...
        while !heap.isEmpty {
            ...
            // heap에 매번 새로운 값을 넣는가? => heap 갱신시 최대 경로 보장 안될 가능성 있음 => 이전 경로 정보가 쌓이기 때문. 초기화 경우 필요
            // 초기화 기준 명확하지 않음. 또한, heap을 사용하므로 초기화 없이 갱신 보장됨
            // heap에 정보가 쌓여도 최대 경로는 보장됨 => 어차피 최대값만 빠져나오기 때문 => 단, 값이 더해져야 할 때 보장이 되는가?
            // 더한 값 때문에 기존 값을 없애야 할 수 있음 => 근데 어떻게?
            // 기존 값을 빼면서 새 값을 더하면 가장 큰값 보장됨. 초기화 없어도 됨
            // 새로 추가되는 값들은 어차피 기존 값보다 큰 값. 다만 경로까지 기억하는가는 별개 => 경로도 함께 저장하는 방법 있음
            // 경로 함께 저장시 heap 사용하면 특정 경로 구분해야할 수 있음
            // heap 사용 자체에 대한 고민이 필요. 너무 heap에 대해서만 고민하고 있는 것 같음. 다른 풀이법은?
            // heap을 사용하지 않는 방법이 있나 확인할 필요 있음
            // heap을 사용한다면 어떻게 경로를 챙길 것인가
            // 경로를 함께 저장해서 필요한 경우만 뽑아서 갱신
        }
