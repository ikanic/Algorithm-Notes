...
public struct PGM118669: Solvable {
    ...
    func solution(_ n: Int, _ paths: [[Int]], _ gates: [Int], _ summits: [Int]) -> [Int] {
        ...
        while !heap.isEmpty {
            ...
            // 2차원 배열 재활용 => 원래 사용하던 걸 1차로 임시 저장, 2차로 최종 결과 저장용으로 => 마지막엔 최소만 있으면 되므로 추가 배열 없이 변수 하나 갱신해서 최장 중 최소 추적 => 이 경우 변수 2개가 필요할 수도 => 최소 자체를 저장 및 경로 저장 => 찾는 것이 전체 경로가 아님 => 최소 출발점 및 도착점
        }
